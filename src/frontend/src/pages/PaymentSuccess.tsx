...
export default function PaymentSuccess() {
  ...
  useInternetIdentity(); // keep auth context initialized
  ...
  useEffect(() => {
    ...
    if (!sessionId) {
      setErrorMessage(
        "No session ID found. Your payment may still have been processed.",
      );
      setStatus("error");
      return;
    }
    ...
    const verifyAndUpgrade = async () => {
      try {
        ...
        if (sessionStatus.__kind__ === "completed") {
          ...
          setErrorMessage(
            sessionStatus.failed?.error || "Payment verification failed.",
          );
          setStatus("error");
        } else {
          setErrorMessage(
    ...
    verifyAndUpgrade();
    // biome-ignore lint/correctness/useExhaustiveDependencies: identity not used inside effect body, only actor is needed
  }, [actor, queryClient]);
