...
export function RazorpaySetupModal({ open, onClose }: RazorpaySetupModalProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      await setConfigMutation.mutateAsync({ keyId: keyId.trim(), keySecret: keySecret.trim() });
      onClose();
    } catch {
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter your Razorpay API credentials to enable INR payments. You can find these in your{' '}
            <a
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Starts with <code>rzp_live_</code> for production or <code>rzp_test_</code> for testing.
            </p>
          </div>
