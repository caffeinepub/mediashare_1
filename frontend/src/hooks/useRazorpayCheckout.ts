...
export function useRazorpayCheckout() {
  ...
  return useMutation({
    mutationFn: async ({ amountInPaise }: { keyId?: string; amountInPaise: number }) => {
      ...
      if (typeof actorAny.createRazorpayOrder !== 'function') {
        throw new Error('Razorpay order creation is not yet available. Please contact support.');
      }

      const orderResult = await actorAny.createRazorpayOrder(BigInt(amountInPaise));
      const order = JSON.parse(orderResult) as { id: string; amount: number; currency: string };
